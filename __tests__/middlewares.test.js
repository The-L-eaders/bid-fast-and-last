...
xdescribe('Auth Middleware', () => {
  ...
  it('Login', async () => {
    let userLogin = await mock.post('/logIn').send({ email: 'user@use.com', password: '123' });
    expect(userLogin.status).toEqual(302);

  });

  it('Login', async () => {
    let userLogin = await mock.post('/logIn').send({ email: 'user@use1.com', password: '123' });
    expect(userLogin.status).toEqual(500);
  });
  it('404', async () => {
    let userLogin = await mock.post('/blablabla').send({ email: 'user@use1.com', password: '123' });
  ...
  it('Bearer', async () => {
    ...
    let car = await mock.get('/car').set(`Authorization`, `Bearer ${last_last[0]}`);
  ...
  it('add Car', async () => {
    ...
    const data = {
      ...
      "image": "https://www.bmw-me.com/content/dam/bmw/common/home/teaser/bmw-2-series-gran-coupe-inspire-ag-home-teaser-xxl.jpg",
      "category": "car",
      "timer": 120
    }
    let car = await mock.post('/add').send(data).set(`Authorization`, `Bearer ${last_last[0]}`);
  ...
  it('get House', async () => {
    ...
    let last = token.split('=');
    let last_last = last[1].split(';');
    let car = await mock.get('/house').set(`Authorization`, `Bearer ${last_last[0]}`);
    expect(car.status).toEqual(200)
  });
  ...
  it('category', async () => {
    ...
    let last = token.split('=');
    let last_last = last[1].split(';');
    let category = await mock.get('/category').set(`Authorization`, `Bearer ${last_last[0]}`);
  ...
  it('add', async () => {
    ...
    let last = token.split('=');
    let last_last = last[1].split(';');
    let add = await mock.get('/add').set(`Authorization`, `Bearer ${last_last[0]}`);
  ...
  it('Owner cant bid in his Own Car',async()=>{
    ...
    let carPage = await mock.get('/car').set(`Authorization`, `Bearer ${last_last[0]}`);
    // console.log(carPage);
  ...
  it('Owner cant bid in his Own House',async()=>{
    ...
    let car = await mock.post('/add').send(data).set(`Authorization`, `Bearer ${last_last[0]}`);
    let carPage = await mock.get('/house').set(`Authorization`, `Bearer ${last_last[0]}`);
    // console.log(carPage);
    expect(carPage.text).toEqual('You Cant bid in your own Product');
    expect(carPage.body).toEqual({});
  });
